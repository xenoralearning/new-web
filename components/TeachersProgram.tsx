...
const TeachersProgram: React.FC<TeachersProgramProps> = ({ onNavigate }) => {
  ...
  return (
    ...
    <section id="teachers" className="relative py-16 sm:py-20 md:py-24 lg:py-32 xl:py-40 2xl:py-48 px-4 sm:px-6 lg:px-12 xl:px-16 z-10 w-full">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 lg:gap-16 xl:gap-24 items-center">
        ...
        {/* Content (~55%) - Enhanced desktop spacing */}
        <div className="lg:col-span-7">
          <Reveal delay={300}>
            <div className="inline-block mb-4 lg:mb-6 px-3 py-1 rounded-full bg-blue-500/10 text-blue-500 font-semibold text-xs lg:text-sm tracking-widest uppercase">
                {t.badge}
            </div>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-display font-bold text-slate-900 dark:text-white mb-6 sm:mb-8 lg:mb-10 xl:mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 xl:gap-10">
            {t.features.map((feature, idx) => (
              <Reveal key={idx} delay={300 + (idx * 50)}>
                  <div className="flex gap-3 sm:gap-4 lg:gap-5 xl:gap-6 p-4 lg:p-6 xl:p-8 rounded-xl lg:rounded-2xl bg-white/50 dark:bg-white/5 border border-slate-200 dark:border-white/5 hover:border-blue-500/30 hover:bg-white/80 dark:hover:bg-white/10 transition-all duration-300 hover:-translate-y-1">
                    <CheckCircle2 className="w-5 sm:w-6 lg:w-7 xl:w-8 h-5 sm:h-6 lg:h-7 xl:h-8 text-blue-500 flex-shrink-0 mt-0.5" />
                    <div className="min-w-0">
                      <h4 className="font-bold text-sm sm:text-base lg:text-lg xl:text-xl text-slate-800 dark:text-white mb-1 lg:mb-2">{feature.title}</h4>
                      <p className="text-xs sm:text-sm lg:text-base xl:text-lg text-slate-600 dark:text-slate-400 leading-snug">{feature.description}</p>
          ...
          <Reveal delay={600}>
            ...
            <div className="mt-8 sm:mt-10 lg:mt-12 xl:mt-16">
                <button 
                  onClick={onNavigate}
                  className="px-6 sm:px-8 lg:px-10 xl:px-12 py-3 sm:py-4 lg:py-5 xl:py-6 rounded-lg lg:rounded-xl border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white transition-all font-semibold uppercase tracking-wider text-sm lg:text-base xl:text-lg hover:shadow-lg hover:shadow-blue-500/30 active:scale-95 min-h-[48px] lg:min-h-[56px] xl:min-h-[64px] w-full sm:w-auto"
